...
import { globalLog } from "../utils/logs";
...
export default class Page extends EventEmitter {
  ...
  create() {
    ...
    each(this.selectors.elements, (selector, key) => {
      ...
      if (
        ...
        globalLog("Selector is element", selector);
        this.elements[key] = selector;
      } else if (Array.isArray(selector)) {
        globalLog("Selector is array of elements", selector);
        this.elements[key] = selector;
      } else {
        globalLog(`Selector === Text`);
        globalLog("Select ALL", selector);
        this.elements[key] = this.element.querySelectorAll(selector);

        if (this.elements[key].length === 0) {
          globalLog(`No elements - unsetting key`);
          this.elements[key] = null;
        } else if (this.elements[key].length === 1) {
          globalLog("Single Selector", selector);
          this.elements[key] = this.element.querySelector(selector);
        }

        globalLog("Selected elements: ", this.elements[key]);
      }
    });
  ...
  createObserver() {
    if (!this.elements.wrapper) {
      globalLog(`No Observer wrapper found`);
      return;
    }

    if (this.elements.wrapper.length > 1) {
      globalLog(`Observer wrapper must be a single element`);
      return;
    }
    // globalLog("this.elements", this.elements);
